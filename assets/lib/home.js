...
function setStats() {
    $.getJSON("https://api.commonwealth.gg/chart/info", function (data) {
		if (data !== null){
			...
			$("#etcPriceUSD").replaceWith(data.ETHPriceUSD)

			// $('#createdStats').popup({
			// 	html: 'EWLTH tokens can <b>only</b> be created by sending Ethereum Classic (ETH) to the contract. There is no central authority that can inflate P3C. Hard cap assumes every ETH possible (210,000,000) is in contract.',
			// 	position: 'bottom center'
			// });
			// $('#priceStats').popup({
			// 	html: 'Current price of a new P3C token. All tokens are denominated in ETH and feed into the locked fund.',
			// 	position: 'bottom center'
			// });
			// $('#sizeStats').popup({
